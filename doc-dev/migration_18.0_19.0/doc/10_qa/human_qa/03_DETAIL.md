...
> **Catatan penting sebelum mulai:** beberapa perilaku di bawah ini adalah BUG YANG DIKETAHUI dan SENGAJA DIPERTAHANKAN dari baseline 18.0 (bukan sesuatu yang perlu dilaporkan ulang sebagai temuan baru) — lihat `01b_BASELINE_SPEC.md` `[BSL-013]`/`[BSL-014]`. Tujuan checklist ini adalah memastikan behavior-nya **tetap sama** seperti sebelumnya, bukan mengecek "apakah sudah benar".
...
- Baris multi-currency: kolom `Total` (core) sudah dikonversi ke mata uang perusahaan, TAPI "Amount Received"/"Waiting for Payment"/"Amount To Invoice" TIDAK dikonversi — angkanya jadi tidak "nyambung" secara matematis dengan kolom Total di baris yang sama. **Ini bug `[BSL-014]` yang sudah diketahui sejak baseline 17.0/18.0, JANGAN dilaporkan sebagai bug baru** — cukup konfirmasi masih berperilaku sama.
